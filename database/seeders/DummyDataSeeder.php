...
class DummyDataSeeder extends Seeder
{
    ...
    {
        DB::table('dummy_data_dc_new')->truncate();

        $data = $this->parseSqlData('dummy_data_dc_new');

        if (!empty($data)) {
            // Use array_chunk to insert in batches of 100 to prevent query parameter limits and run faster
            foreach (array_chunk($data, 100) as $chunk) {
                DB::table('dummy_data_dc_new')->insert($chunk);
            }
        }
    }
